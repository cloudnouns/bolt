...
function crc32(buffer: Uint8Array, offset: number, size: number) {
	let crc = -1;
...
export default class PNGImage {
	...
	constructor(opts: {
		...
		const { width, height, depth, bgColor } = opts;
		...
		this.bgColor = this.createColor(bgColor ?? 'transparent');
	}
	...
	createColor(hex: HexColor) {
		const color = colord(hex);

		let rgb;
		if (hex === 'transparent') rgb = color.alpha(0).toRgb();
		else rgb = color.toRgb();
